        ...
        <div class="post_bottom_area d-flex">
          <div class="d-flex post_status">
            ...
            <div>
              ...
              @else
              <p class="m-0"><i class="far fa-heart like_btn" post_id="{{ $post->id }}"></i><span class="like_counts{{ $post->id }}">{{ $post->likeCount() }}</span></p>
              @endif
            </div>
      </div>
      ...
      <div class="accordion">
        <!-- 登録されているメインカテゴリー -->
            @foreach($categories as $category)
                  <label class="main_categories" category_id="{{ $category->id }}"><span>{{ $category->main_category }}<span></label>
          <!-- 登録されているサブカテゴリー -->
          <!-- サブカテゴリーのボタンを押すとサブカテゴリーに属する投稿が表示される -->
          <ul>
            <li class="sub_categories" sub_category_id="{{ $category->id }}">
